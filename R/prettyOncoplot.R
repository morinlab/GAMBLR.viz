prettyOncoplot <- function(maf_df,
  ...
  if (missing(genes)) {
    ...
    if (!recycleOncomatrix) {
      ...
      mat_origin <- create_onco_matrix(maf_df, genes)
  ...
  summarize_mutation_by_class <- function(mutation_set) {
    if ("hot_spot" %in% mutation_set) {
      snv_maf <- filter(maf_df, hot_spot == TRUE) %>%
        ...
        unique() %>% # keep at most one per group/gene combo
        mutate(Mutated = TRUE)
    } else {
...
if (annotate_specific_genes & is.null(this_forest_object)) {
  ...
  right_annotation <- rowAnnotation(
    " " = enrichment_label,
    col = list(" " = c(get_gambl_colours()[these_comparisons],
...
make_prettyoncoplot <- function(mat_input,
  ...
  if (plot_type == "simplify") {
    ...
    heatmap_legend_param <- list(
      title = "Alterations",
      at = c(
        "RNA", "3'UTR", "5'UTR", "Nonsense_Mutation", "Splice_Site", "Splice_Region", "Nonstop_Mutation", "Translation_Start_Site",
        "In_Frame_Ins", "In_Frame_Del", "Frame_Shift_Ins", "Frame_Shift_Del", "Multi_Hit", "Missense_Mutation", "Silent", "hot_spot"
      ),
      labels = c(
        "RNA", "3'UTR", "5'UTR", "Nonsense Mutation", "Splice Site", "Splice Region", "Nonstop Mutation", "Translation Start Site",
  ...
  oncoprint_args[["bottom_annotation"]] <- ComplexHeatmap::HeatmapAnnotation(
    df = metadata_df,
    show_legend = show_legend,
    col = colours,
    simple_anno_size = unit(metadataBarHeight, "mm"),
