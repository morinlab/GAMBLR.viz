...
#' @param return_mutmat Optional flag to return the mutation matrix generated by the function
#' 
#' @return By default, nothing unless return_mutmat is specified, 
...
prettyOncoplot = function(maf_df,
  ...
  if(simplify){
    ...
    splice_df[trunc_df==TRUE] = FALSE
    #if(!missing(cnv_df)){
    #  cn_df[trunc_df==TRUE | splice_df==TRUE | snv_df == TRUE] = FALSE
    #}
  ...
  for(annotation in names(colours)){
    all_names = pull(metadata_df,annotation) %>% unique() %>% as.character()
  ...
  if(simplify){
    ...
    if(cluster_rows | cluster_cols){
      if(!cluster_cols){
        ...
        h_obj = pheatmap(any_hit,cluster_rows=T,cluster_cols=T,
                         clustering_distance_rows = clustering_distance_rows,
                         fontsize_row = 6,show_colnames = F)
      }
      if(dry_run){
    ...
    if(!missing(cnv_df)){
      mat_list = list(CNV = as.matrix(cn_df[genes_kept,patients_kept]),
