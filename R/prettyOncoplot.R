...
prettyOncoplot = function(
    ...
    if(!missing(gene_cnv_df)){
        ...
        cnv_df = gene_cnv_df[rownames(gene_cnv_df) %in% patients,,drop=FALSE]
        if(verbose){
            print("AFTER")
            print(dim(cnv_df))
    ...
    if(simplify_annotation){
        ...
        Missense = function(x, y, w, h){ grid.rect(x, y, w * width_adj, h*0.9,
                                                gp = gpar(fill = col["Missense"], col = NA))},
        Truncating = function(x, y, w, h) {grid.rect(x, y, w * width_adj, h*0.9,
                                                    gp = gpar(fill = col["Truncating"], col = NA))},
        Splice_Site = function(x, y, w, h) {grid.rect(x, y, w * width_adj, h*0.9,
                                                    gp = gpar(fill = col["Splice_Site"], col = NA))},

        CNV = function(x, y, w, h) {grid.rect(x, y, w * width_adj, h*0.5,
                                                gp = gpar(fill = col["CNV"], col = NA))},
        HotSpot = function(x, y, w, h) {grid.rect(x, y, w * width_adj, h*0.9,
                                                    gp = gpar(fill = col["Missense"], col = NA));
                                        grid.rect(x, y, w * width_adj, h*0.3,
    ...
    if(simplify_annotation){
        ...
        if(!missing(sortByGenes) & !keepSampleOrder){
          ...
          patients_kept = colnames(any_hit)
            ...
            metadata_df = metadata_df[order(match(rownames(metadata_df),patients_kept)),]
            col_order = patients_kept
        }else {
...
make_prettyoncoplot = function(mat_input,
  ...
  #chh = draw(ch, heatmap_legend_side = legend_position, annotation_legend_side = legend_position)
  ch_ord = ch@column_order
  #chh_ord = column_order(chh)
  ...
  if(return_inputs){
    ...
    return(list(Heatmap=ch,mut_mat=mat_input,metadata=metadata_df,  sample_order = ch_ord))
