...
mutation_hotspot_logo <- function(maf_df,
  ...
  if (missing(fasta_path)) {
    ...
    if (is.null(fasta_path)) {
      ...
      if (bsgenome_name %in% installed) {
        ...
        print(installed)
        print(paste("Local genome not found and missing genome_build",
                    bsgenome_name,
                    "Supply a fastaPath for a local fasta file or",
                    "install the missing BSGenome package and re-run"))
      }
    }
  }else{
    if(!file.exists(fasta_path)){
      stop(paste("cannot find genome fasta file:",fasta_path))
    }
  }
  ...
  if (nrow(maf_df_in_region) == 0) {
    ...
    print(paste0(
      "mutation coordinate range: ",
      maf_df$Chromosome[1], ":",
      min(maf_df$Start_Position), "-",
      max(maf_df$Start_Position)
    ))
    stop(paste(
      "no mutations in maf_df are in the",
      "padded region around the specified coordinate"
    ))
  }

  some_mutations <- filter(
    mut_noindel,
    Start_Position <= end,
    Start_Position >= start
  ) %>%
  ...
  count_bases <- function(original,
                          index, newbase,
                          base_counts,
                          exclude_unmutated = TRUE) {
  ...
  base_counts[] <- 0
  for (i in seq_len(nrow(some_mutations))) {
    base_counts <- count_bases(
      as.character(some_mutations[i, "unmut"]),
  ...
  if (include_reference) {
    if (annotate_motif) {
      ...
      p2 <- ggplot(aln_df, aes(rel_start, 1)) +
        geom_text(aes(label = Reference_Allele,
                      color = WRCY),
                      size = base_size) +
        common_scale +
        xlab("") +
    ...
    if (include_AA | group_AA) {
      if (group_AA) {
        aa_df <- some_mutations %>%
          dplyr::select(AA, rel_start) %>%
          unique()
      } else {
        aa_df <- some_mutations %>%
          dplyr::select(AA, rel_start) %>%
...
plot_signature <- function(maf_df,
                           genes = "MYC",
                           return_data = FALSE,
                           separate_silent = FALSE) {
  all_mutations <- expand_grid(ref = c("T", "C"),
                               alt = c("A", "C", "T", "G")) %>%
    mutate(mutation = paste0(ref, ">", alt))
  all_contexts <- expand_grid(up = c("A", "C", "T", "G"),
                              ref = c("T", "C"),
                              alt = c("A", "C", "T", "G"),
                              down = c("A", "C", "T", "G"))
  all_contexts <- mutate(all_contexts, seq = paste0(up, ref, down),
  ...
  keep = c("Silent", "5'UTR", "3'UTR", "5'Flank", "3'Flank")
  silent_mutations <- dplyr::filter(all_mutations,
                                    Variant_Classification %in% keep)
  all_count <- group_by(all_mutations,
                        mutation,
                        seq,
                        Hugo_Symbol) %>% tally()
  silent_count <- group_by(silent_mutations,
                           mutation,
                           seq,
                           Hugo_Symbol) %>% tally()
  ...
  if (separate_silent) {
    ...
    silent_count <- mutate(silent_count,
                           Hugo_Symbol = paste0(Hugo_Symbol, "-Silent"))
  ...
  ggplot(all_count, aes(x = mutation_context, y = gene_norm, fill = mutation)) +
    geom_col() +
