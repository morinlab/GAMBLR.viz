...
#'
#' @param this_seg Parameter with copy number df already loaded into R.
#' @param this_seg_path Parameter with path to external cn file.
...
#'
fancy_ideogram = function(gene_annotation,
                          this_seg,
                          this_seg_path = NULL,
                          this_maf,
                          this_maf_path = NULL,
  ...
  #avoid non-sense parameter combinations
  if(!include_ssm){
    ssm_count = FALSE
  }
  ...
  if(!missing(gene_annotation)){
    #TODO: update the package to. point to the home of gene_to_region
    gene = GAMBLR.package_name::gene_to_region(gene_symbol = gene_annotation, genome_build = "grch37", return_as = "df")
  ...
  if(include_ssm){
    ...
    if(missing(this_maf) && is.null(this_maf_path)){
